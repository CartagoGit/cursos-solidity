...
contract contractNotas {
    ...
    mapping(bytes32 => uint256) mapNotas;

    // Array de alumnos que pidan revision de examenes
    string[] revisiones;

    // Eventos
    event event_alumno_evaluado(bytes32, uint256);
    ...
    modifier onlyProfesor(address _direccion) {
        require(
            profesor == _direccion,
            "Solo el profesor puede evaluar a los alumnos"
        );
        _;
    }

    // Visualizar notas
    function getNotas(string memory _idAlumno) public view returns (uint256) {
        //Hash del alumno
        bytes32 hash_idAlumno = keccak256(abi.encodePacked(_idAlumno));
        // Nota asociada al hash del alumno
        uint256 nota_alumno = mapNotas[hash_idAlumno];
