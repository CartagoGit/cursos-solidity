...
contract loteria {
    ...
    uint256 tokens_creados = 10000000;

    //Eventos
    event eventTokenComprado(uint256, address);
    ...
    function generaTokens(uint256 _numTokens) public onlyOwner {
    ...
    function balanceBote() public view returns (uint256) {
        return token.balanceOf(owner);
    }

    // Ver los tokens que se disponen en el bolsillo
    function getTokensPerson() public view returns (uint256) {
    ...
    mapping(address => uint256[]) map_idPersona_boletos;
    //Relacion nevesaria para identificar al ganador
    mapping(uint256 => address) map_ganador;
    //Numero aleatorio
    uint256 randNonce = 0;
    //Boletos generados
    uint256[] boletos_comprados;
    //Eventos
    event event_boleto_comprado(uint256, address); //al comprar un boleto
    event event_boleto_ganador(uint256); //evento del ganador
    event event_tokens_devueltos(uint256, address); // evento al devolver los tokens

    //Funcion para comprar boletos
    function comprarBoleto(uint256 _boletos) public {
        //Precio total de los colegos a comprar
        uint256 precio_total_boletos = _boletos * precioBoleto;
        //Filtrado de los tokens a pagar
        require(
            precio_total_boletos <= getTokensPerson(),
            "No tienes tokens suficientes para comprar tanto boletos"
        );
    ...
    function getBoletosPerson() public view returns (uint256[] memory) {
    ...
    function getBackTokens(uint256 _numTokens) public payable {
        ...
        require(
            getTokensPerson() >= _numTokens,
            "Debes disponer del numero de tokens que deseas devolver"
        );
        //DEVOLUCION:
