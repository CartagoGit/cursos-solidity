...
contract centerContract {
    ...
    */
    ...
    modifier onlyThisCenter() {
        require(
            msg.sender == addressCenter,
            "Solo puede ser controlado por el centro de salud correspondiente"
        );
        _;
    }

    // Funcion para emitir un resultado de una prueba covid
    function resultPcrCovid(
        string memory _idPersona,
        bool _resultCovid,
        string memory _ipfsCode
    ) public onlyThisCenter {
    ...
    function visualizeResultsCovid(string memory _idPersona)
        public
        view
