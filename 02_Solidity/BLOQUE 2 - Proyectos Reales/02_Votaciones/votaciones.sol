...
contract votacion {
    ...
    mapping(string => uint256) mapVotosCandidato;
    ...
    function presentarCandidatura(
        ...
        bytes32 hash_candidato = keccak256(
            abi.encodePacked(_nombre, _edad, _idPersona)
        );
    ...
    function votar(string memory _candidato) public {
        ...
        for (uint256 i = 0; i < votantes.length; i++) {
            require(
                votantes[i] != hash_votante,
                "Ya has votado. No puedes votar mas veces"
            );
        }
    ...
    function getVotosCandidato(string memory _candidato)
        public
        view
        returns (uint256)
    {
    ...
    function resultadoVotaciones() public view returns (string[] memory) {
        uint256 numCandidatos = candidatos.length;
        ...
        for (uint256 i = 0; i < candidatos.length; i++) {
            resultados[i] = string(
                abi.encodePacked(
                    candidatos[i],
                    ": ",
                    uint2str(getVotosCandidato(candidatos[i]))
                )
            );
        }
        return resultados;
    ...
    function resultadoGanador() public view returns (string memory) {
        ...
        uint256 votos_maximos = 0;
        for (uint256 i = 0; i < candidatos.length; i++) {
            uint256 votos_candidato = getVotosCandidato(candidatos[i]);
            if (votos_candidato > votos_maximos) {
                votos_maximos = votos_candidato;
                ganador = candidatos[i];
                textoGanador = string(
                    abi.encodePacked("El ganador es: ", ganador)
                );
            } else if (votos_candidato == votos_maximos) {
                ganador = string(
                    abi.encodePacked(ganador, " + ", candidatos[i])
                );
                textoGanador = string(
                    abi.encodePacked("Hay un empate entre: ", ganador)
                );
            }
        }
        return textoGanador;
